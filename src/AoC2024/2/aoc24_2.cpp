...
bool checkSorting(const std::vector<int>& row) {
    ...
    const bool isBackwardSorted = std::ranges::is_sorted(row, std::ranges::greater{});
...
std::vector<int> calculateSafetyLevel(const std::vector<int>& row) {
    ...
    differences.erase(differences.begin()); // first element contains original value

    std::for_each(differences.begin(), differences.end(), [](int& num){
        num = std::abs(num);
    });
